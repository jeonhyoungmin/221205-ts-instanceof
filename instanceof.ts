...
// * * 일반 객체 리터럴과 달리 '인스턴스'라는 것을 확인할 수 있다.
console.log(firstInstance instanceof First); // * true
/**
 *  * typeof() 연산자로 데이터타입을 확인 할 수 있는 것처럼
 *  * class, 즉 생성자 함수로부터 만들어진 객체인지(인스턴스) 확인하는 연산자를 제공한다.
 *  * 좌항(피연산자, 인스턴스) instanceof 우항(클래스)가 바로 그것
 * 해당 인스턴스가 우항의 클래스에서 생성된 객체가 맞다면 true를 리턴한다.
 *
 *  ? 생각해 볼 중요한 관점
 *  pseudo .1 애플리케이션 제작을 위해 원하는 생성자함수로부터 만들어진것이 아니라면 '거부'시키는 방식
 *  pseudo .2 위의 First 생성자 함수는 재사용에 대한 의도가 전혀 없기 때문에 쓸모가 없다.
 *  pseudo .3 다시말해 재사용성을 고려한 모듈, '검사'를 위한 객체 조사여부라면 상당히 신뢰도 높은 값을 얻어 낼 수 있다.
 *  pseudo .4 신뢰도 === 안정성 === 타입스크립트의 철학 이 세 가지의 목적이 맞물려 있다.
